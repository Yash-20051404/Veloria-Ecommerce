...
import { Package, CheckCircle, AlertCircle, Eye as EyeOff, Search, SlidersHorizontal, RotateCcw, Plus, Pencil, Eye, Trash2, ChevronLeft, ChevronRight, ChevronUp } from 'lucide-react';
                ...
                <tr key={p.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-3 py-3">
                    <span className={`inline-flex items-center gap-1.5 text-xs font-medium`}>
                      <span className={`w-2 h-2 rounded-full ${statusDot[p.status ?? "Draft"]}`} />
                      <span className={statusDot[p.status ?? "Draft"]}>{p.status}</span>
                    </span>
                  </td>
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button onClick={() => setDeleteId(p.id ?? null)} className="text-gray-400 hover:text-red-500 transition-colors"><Trash2 size={15} /></button>
                    </div>
                  </td>
                </tr>
              ))}
