...
export default function Customers() {
  ...
  const realCustomers = useMemo(() => {
    ...
    orders.forEach((o: any) => {
      const email = (o.email || 'unknown@example.com').toLowerCase();
      if (!map.has(email)) {
        map.set(email, {
          id: email,
          ...
          orders: 0,
          totalSpent: 0,
          memberSinceDate: new Date(o.createdAt || Date.now()),
          status: 'Active',
          initials: String(o.customerName || 'U').charAt(0).toUpperCase(),
      ...
      if (oDate < c.memberSinceDate) c.memberSinceDate = oDate;
    });
    return Array.from(map.values()).map(c => ({
      ...c,
      memberSince: c.memberSinceDate.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })
    }));
  }, [orders]);
