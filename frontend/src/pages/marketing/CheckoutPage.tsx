...
const DeliveryOptions = memo(({ selected, onSelect }: { selected: string, onSelect: (val: string) => void }) => {
  const options = [
    { id: 'complimentary', title: 'Complimentary Insured Delivery', desc: 'Arrives in 2-3 business days', price: 'Free', icon: ShieldCheck },
...
const OrderSummary = memo(({ 
  ...
  const estimatedArrival = useMemo(() => {
    ...
    const d2 = new Date(); d2.setDate(d2.getDate() + (deliveryCost === 50 ? 2 : 3));
...
setSelectedAddressId(defaultAddr.id || defaultAddr._id || null);
    }
  ...
  const handleSaveNewAddress = async () => {
    ...
    if (!formData.fullName || !formData.phone || !formData.addressLine1 || !formData.city || !formData.state || !formData.pincode) {
      ...
      if (!formData.fullName) missing.fullName = 'Full name is required';
      if (!formData.phone) missing.phone = 'Phone number is required';
      if (!formData.addressLine1) missing.addressLine1 = 'Address is required';
      if (!formData.city) missing.city = 'City is required';
      if (!formData.state) missing.state = 'State is required';
      if (!formData.pincode) missing.pincode = 'Postal code is required';
    ...
    const result = await addAddress(currentUserId, formData);
    ...
    if (result.success) {
      ...
      // Show field-specific validation errors below each field
      ...
      setToast({ show: true, message: result.message || 'Validation failed. Please check the fields below.' });
    }
  }
  ...
  <div className="border border-[#D6B57A]/20 bg-[#D6B57A]/5 p-6">
    <p className="text-[10px] uppercase tracking-widest text-[#D6B57A] mb-2" style={inter}>
      Using Maison Account
                ...
                <p className="mt-1 text-white/60 text-sm" style={inter}>{profile?.phone || (user as any)?.phone || 'No phone number provided'}</p>
    ...
    <LuxuryInput label="Mobile Number" type="tel" required />
              ...
              <section className="mt-12 border-t border-white/10 pt-12">
                ...
                <div className="space-y-4 mb-8">
                  {Array.isArray(addresses) && addresses.filter(a => a?.id || a?._id).length > 0 ? (
                    addresses.filter(a => a?.id || a?._id).map((addr: any, idx: number) => (
                      <button
                    ...
                    {!addressConfirmed ? (
                      <div className="space-y-10">
                        ...
                        <div>
                          ...
                          <LuxuryInput label="Full Name" required value={formData.fullName} onChange={(e: any) => setFormData({...formData, fullName: e.target.value})} />
                              {formErrors.fullName && <p className="mt-2 text-xs text-red-400">{formErrors.fullName}</p>}
                            </div>
                            <div className="md:col-span-1">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="md:col-span-2">
                              ...
                              <select value={formData.country} onChange={(e) => setFormData({ ...formData, country: e.target.value })} className="w-full appearance-none border-b border-white/20 bg-transparent px-0 py-4 text-sm text-white focus:border-[#D6B57A] focus:outline-none focus:ring-0">
                                <option value="India" className="bg-[#050505]">India</option>
                                <option value="United States" className="bg-[#050505]">United States</option>
                                <option value="United Kingdom" className="bg-[#050505]">United Kingdom</option>
                                <option value="Canada" className="bg-[#050505]">Canada</option>
                                <option value="Australia" className="bg-[#050505]">Australia</option>
                              </select>
                            </div>
                            <div>
                            ...
                            <LuxuryInput label="ZIP / Postal Code" required value={formData.pincode} onChange={(e: any) => setFormData({...formData, pincode: e.target.value})} />
                                {formErrors.pincode && <p className="mt-2 text-xs text-red-400">{formErrors.pincode}</p>}
                                <p className="mt-2 text-[11px] text-white/40" style={inter}>
                                  Example: 201301
                                </p>
                              </div>
                        ...
                        <div>
                          ...
                          <LuxuryInput label="House No., Apartment, Building Name" required value={formData.addressLine1} onChange={(e: any) => setFormData({...formData, addressLine1: e.target.value})} />
                              {formErrors.addressLine1 && <p className="mt-2 text-xs text-red-400">{formErrors.addressLine1}</p>}
                              <p className="mt-2 text-[11px] text-white/40" style={inter}>
                          ...
                          <LuxuryInput label="Street, Area, Landmark" value={formData.addressLine2} onChange={(e: any) => setFormData({...formData, addressLine2: e.target.value})} />
                              <p className="mt-2 text-[11px] text-white/40" style={inter}>
