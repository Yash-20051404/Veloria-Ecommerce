...
import { 
  ...
  Search, User, Star, CheckCircle, Share2, Loader2
} from 'lucide-react'
      ...
      <div className="mt-12 flex flex-col gap-4 sm:flex-row sm:items-center">
        <div className="flex h-14 w-32 items-center justify-between border border-white/20 px-4 text-white">
          ...
          <button 
            ...
            className="text-white/50 hover:text-white"><Plus className="h-3 w-3" /></button>
        </div>
...
function SimilarProducts({ currentId }: { currentId: string }) {
  const navigate = useNavigate()
  const { similarProducts, similarLoading, fetchSimilarProducts } = useProductStore()

  useEffect(() => {
    if (currentId) fetchSimilarProducts(currentId)
  }, [currentId, fetchSimilarProducts])
      ...
      <section className="w-full bg-[#030303] px-6 py-32 lg:px-16">
        <div className="mx-auto max-w-[1600px]">
          <div className="mb-16 text-center md:text-left">
            <h3 className="text-3xl font-light text-white md:text-4xl" style={cormorant}>Similar Products</h3>
            <p className="mt-2 text-xs uppercase tracking-widest text-[#D6B57A]" style={inter}>AI Curated</p>
          </div>
          <div className="flex justify-center py-20">
            <Loader2 className="h-8 w-8 animate-spin text-[#D6B57A]" />
          </div>
        </div>
      </section>
    )
    ...
    <section className="w-full bg-[#030303] px-6 py-32 lg:px-16">
      <div className="mx-auto max-w-[1600px]">
        <div className="mb-16 text-center md:text-left flex flex-col md:flex-row justify-between items-center">
          <div>
            <h3 className="text-3xl font-light text-white md:text-4xl" style={cormorant}>Similar Products</h3>
            <p className="mt-2 text-xs uppercase tracking-widest text-[#D6B57A]" style={inter}>AI Curated • Semantic Match</p>
          </div>
        ...
        <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {similarProducts.slice(0, 8).map((prod, idx) => (
            <motion.article
              key={prod.id}
              onClick={() => {
            ...
            >
              ...
              <div className="mt-5 text-center">
                <p className="text-[9px] uppercase tracking-[0.3em] text-[#D6B57A]" style={inter}>{prod.category || 'Collection'}</p>
...
function YouMayAlsoLike() {
  const navigate = useNavigate()
  const { recommendations, recommendationsLoading, fetchRecommendations } = useProductStore()
  const wishlistItems = useWishlistStore(state => state.items)
  const cartItems = useCartStore(state => state.items)

  useEffect(() => {
    // Collect product IDs from wishlist and cart for personalized recommendations
    const historyIds = [
      ...wishlistItems.map(i => String(i.id)),
      ...cartItems.map(i => String(i.id)),
    ]
    // Deduplicate
    const uniqueIds = [...new Set(historyIds)]
    fetchRecommendations(uniqueIds.length > 0 ? uniqueIds : undefined)
  }, [fetchRecommendations, wishlistItems.length, cartItems.length])

  if (recommendationsLoading) {
    return (
      <section className="w-full bg-[#050505] px-6 py-32 lg:px-16">
        <div className="mx-auto max-w-[1600px]">
          <div className="mb-16 text-center md:text-left">
            <h3 className="text-3xl font-light text-white md:text-4xl" style={cormorant}>You May Also Like</h3>
            <p className="mt-2 text-xs uppercase tracking-widest text-[#D6B57A]" style={inter}>Personalized using AI</p>
          </div>
          <div className="flex justify-center py-20">
            <Loader2 className="h-8 w-8 animate-spin text-[#D6B57A]" />
          </div>
  ...
  return (
    <section className="w-full bg-[#050505] px-6 py-32 lg:px-16">
      <div className="mx-auto max-w-[1600px]">
        ...
        <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {recommendations.slice(0, 8).map((prod, idx) => (
            <motion.article
              key={prod.id}
              onClick={() => {
                navigate(`/product/${prod.id}`);
