  ...
  persist(
    (set, get) => ({
      ...
      addToCart: (item) => {
        ...
        if (existingItem) {
          const newQty = existingItem.quantity + Math.max(1, item.quantity || 1);
          const updatedItems = items.map((i) =>
            i.id === item.id
              ? { ...i, quantity: i.stock !== undefined ? Math.min(newQty, i.stock) : newQty }
              : i
          );
          set(calculateDerivedState(updatedItems));
        } else {
          const initialQty = Math.max(1, item.quantity || 1);
          const updatedItems = [
            ...items,
            { ...item, quantity: item.stock !== undefined ? Math.min(initialQty, item.stock) : initialQty },
          ];
          set(calculateDerivedState(updatedItems as CartItem[]));
        }
      },
      ...
      increaseQuantity: (id) => {
        const updatedItems = get().items.map((i) =>
          i.id === id ? { ...i, quantity: i.stock !== undefined ? Math.min(i.quantity + 1, i.stock) : i.quantity + 1 } : i
        );
        set(calculateDerivedState(updatedItems));
      },
