    ...
    (set, get) => ({
      ...
      login: async (credentials) => {
        ...
        if (user._id && !user.id) user.id = user._id;
          ...
          // The cart/wishlist stores are shared per-browser (not per-account) in
          // localStorage, so if a different account logs in on this device, the
          // previous account's cart/wishlist must not carry over.
