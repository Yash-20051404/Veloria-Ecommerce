...
export class CartController {
  ...
  static updateCartItem = asyncHandler(async (req: Request, res: Response) => {
    // @ts-ignore
    const userId = req.user._id.toString();
    const cart = await cartService.updateCartItem(userId, String(req.params.productId), req.body.quantity);
  ...
  static removeCartItem = asyncHandler(async (req: Request, res: Response) => {
    // @ts-ignore
    const userId = req.user._id.toString();
    const cart = await cartService.removeCartItem(userId, String(req.params.productId));
