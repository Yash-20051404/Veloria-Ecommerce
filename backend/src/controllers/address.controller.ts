...
export class AddressController {
  ...
  static getAddressById = asyncHandler(async (req: Request, res: Response) => {
    // @ts-ignore
    const userId = req.user._id.toString();
    const address = await addressService.getAddressById(String(req.params.id), userId);
  ...
  static updateAddress = asyncHandler(async (req: Request, res: Response) => {
    // @ts-ignore
    const userId = req.user._id.toString();
    const address = await addressService.updateAddress(String(req.params.id), userId, req.body);
  ...
  static deleteAddress = asyncHandler(async (req: Request, res: Response) => {
    // @ts-ignore
    const userId = req.user._id.toString();
    await addressService.deleteAddress(String(req.params.id), userId);
  ...
  static setDefaultAddress = asyncHandler(async (req: Request, res: Response) => {
    // @ts-ignore
    const userId = req.user._id.toString();
    const address = await addressService.setDefaultAddress(String(req.params.id), userId);
