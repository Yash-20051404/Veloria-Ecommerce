...
export class AuthController {
  static async register(req: Request, res: Response, next: NextFunction) {
    // Block anyone from registering as an ADMIN from the public API
    if (req.body.role && req.body.role.toUpperCase() === 'ADMIN') {
      return next(new AppError(403, 'Admin registration is not allowed.'));
    }
  ...
  static async createAdmin(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      await authService.createAdminFromInvitation(token, name, password);
      res.status(202).json({
        success: true,
        message: 'Invitation accepted. Please verify your email with the OTP sent.'
      });
    } catch (error) {
      next(error);
    }
