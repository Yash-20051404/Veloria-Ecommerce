...
class EmailService {
  ...
  async sendEmail(to: string, subject: string, html: string) {
    // Dev convenience: still log so you can see it in the terminal without
    // needing a working inbox while developing locally.
    ...
    if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
      console.error(
        '[email] SMTP_USER / SMTP_PASS are not set — cannot actually send email. ' +
        'Set them in your .env (e.g. a Gmail address + app password, or another SMTP provider).'
      );
      return false;
    }
  ...
  async sendAdminInvitation(to: string, token: string) {
    ...
    const inviteLink = `${process.env.CLIENT_URL}/create-admin?token=${token}`;
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px; background-color: #030303; color: #ffffff; text-align: center;">
        <h1 style="color: #D6B57A; letter-spacing: 0.3em; font-weight: 300; margin-bottom: 30px;">VELORIA</h1>
        <p style="color: #aaaaaa; font-size: 14px; letter-spacing: 0.1em; text-transform: uppercase;">Admin Invitation</p>
        <p style="color: #ffffff; margin-top: 20px;">You have been invited to become an administrator for Veloria. Click the link below to create your account.</p>
        <a href="${inviteLink}" style="display: inline-block; background-color: #D6B57A; color: #030303; padding: 12px 24px; text-decoration: none; font-weight: bold; margin: 30px 0; border-radius: 4px;">Create Your Admin Account</a>
        <p style="color: #aaaaaa; font-size: 11px; letter-spacing: 0.1em; text-transform: uppercase;">This link will expire in 24 hours.</p>
      </div>
    `;
    return this.sendEmail(to, subject, html);
  }
